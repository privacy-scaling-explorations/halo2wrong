...
pub fn modulus<F: PrimeField>() -> Big {
    Big::from_str_radix(&F::MODULUS[2..], 16).unwrap()
}
pub fn power_of_two<F: PrimeField>(n: usize) -> F {
    big_to_fe(Big::one() << n)
}
pub fn big_to_fe<F: PrimeField>(e: Big) -> F {
    let modulus = modulus::<F>();
    let e = e % modulus;
    F::from_str_vartime(&e.to_str_radix(10)[..]).unwrap()
}
pub fn fe_to_big<F: PrimeField>(fe: F) -> Big {
    Big::from_bytes_le(fe.to_repr().as_ref())
}
pub fn decompose<W: PrimeField, N: PrimeField>(
    e: W,
...
pub fn decompose_big<F: PrimeField>(e: Big, number_of_limbs: usize, bit_len: usize) -> Vec<F> {
    let mut e = e;
...
pub fn compose<W: PrimeField, N: PrimeField>(input: Vec<N>, bit_len: usize) -> W {

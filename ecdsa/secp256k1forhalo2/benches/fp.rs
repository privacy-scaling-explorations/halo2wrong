...
fn bench_fp_add_assign(b: &mut Bencher) {
    ...
    let v: Vec<(Fp, Fp)> = (0..SAMPLES).map(|_| (Fp::random(&mut rng), Fp::random(&mut rng))).collect();
...
fn bench_fp_from_repr(b: &mut Bencher) {
    ...
    let v: Vec<<Fp as PrimeField>::Repr> = (0..SAMPLES).map(|_| Fp::random(&mut rng).to_repr()).collect();

...
fn bench_fq_add_assign(b: &mut Bencher) {
    ...
    let v: Vec<(Fq, Fq)> = (0..SAMPLES).map(|_| (Fq::random(&mut rng), Fq::random(&mut rng))).collect();
...
fn bench_fq_from_repr(b: &mut Bencher) {
    ...
    let v: Vec<<Fq as PrimeField>::Repr> = (0..SAMPLES).map(|_| Fq::random(&mut rng).to_repr()).collect();

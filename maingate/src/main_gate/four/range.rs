...
impl<F: FieldExt> RangeInstructions<F> for RangeChip<F> {
    fn range_value(&self, ctx: &mut RegionCtx<'_, '_, F>, input: &UnassignedValue<F>, bit_len: usize) -> Result<AssignedValue<F>, Error> {
        ...
        let assigned = if (number_of_dense_limbs == 1 && fine_limb_bit_len == 0) || number_of_dense_limbs == 0 {
            ...
            let combination_option = CombinationOptionCommon::CombineToNextAdd(-one).into();
            let _ = main_gate.combine(ctx, [term_0, term_1, term_2, term_3], zero, combination_option)?;
            ...
            if has_overflow {
                ...
                // a_4 * R^4 - input + t  = 0
                let (_, _, assigned, _) = main_gate.combine(
                    ctx,
                    [
                        Term::Zero,
                ...
                main_gate.assign_to_acc(ctx, input)
            }
        };
...
mod tests {
    ...
    use crate::main_gate::four::main_gate::MainGate;
    use crate::main_gate::four::NUMBER_OF_LOOKUP_LIMBS;

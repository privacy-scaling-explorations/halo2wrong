...
type CombinedValues<F> = (AssignedValue<F>, AssignedValue<F>, AssignedValue<F>, AssignedValue<F>);
...
impl<F: FieldExt> MainGateInstructions<F, WIDTH> for MainGate<F> {
    ...
    fn decompose(
        ...
        for i in 0..number_of_rounds {
            ...
            acc = match (coeff_0.value(), coeff_1.value(), coeff_2.value(), acc) {
                (Some(c_0), Some(c_1), Some(c_2), Some(acc)) => Some(acc - (base_0 * c_0 + base_1 * c_1 + base_2 * c_2)),
                _ => None,
            };
        }
        ...
        fn synthesize(&self, config: Self::Config, mut layouter: impl Layouter<F>) -> Result<(), Error> {
            ...
            layouter.assign_region(
                ...
                |mut region| {
                    ...
                    let constant = -(a_0 * a_1 + r_0 * a_0 + r_1 * a_1 + a_2 * r_2 + a_3 * r_3 + a_next * r_next);
                    ...
                    let (a_0, a_1, a_2, a_3, a_next) = (rand(), rand(), rand(), rand(), rand());
                    let (r_scale, r_0, r_1, r_2, r_3, r_next) = (rand(), rand(), rand(), rand(), rand(), rand());

                    let constant = -(r_scale * a_0 * a_1 + r_0 * a_0 + r_1 * a_1 + a_2 * r_2 + a_3 * r_3 + a_next * r_next);
                    ...
                    let constant = -(r_0 * a_0 + r_1 * a_1 + a_2 * r_2 + a_3 * r_3 + a_next * r_next);
